...
function getCalendar(a) {
    $.ajax({
        url: "teaching_calendar.php?date1="+a,
...
console.log("Number of stories selected - "+count_check);
if(count_check>0) {
	var vals = "";
	...
	loadStories(vals,div_id);
} else {
	alert("No story selected!!");
}
}

function loadStories(vals,div_id){
...
for(var i = 0; i < val_array.length; i++)
{
	...
	if(!document.getElementById(str_att_id+id_arr[0]))
	{
		...
		console.log(str_att_id+id_arr[0]);
		...
		ele.innerHTML="<p>"+id_arr[0]+" -> "+id_arr[1]+" <span class=\"close\" onclick=\"deleteStory(this)\" value=\""+str_att_id+id_arr[0]+"\" style='float: right;'>&times;</span></p> ";
		output.appendChild(ele);

	}
	
}
console.log(form_story.slice(1));
...
function slideshow(ele) {
	var lesson_id = ele.getAttribute('value');
	window.location.href = "http://memories.tamu.edu/slideshow.php?lesson_id="+lesson_id;
}
</script>
