...
.mainNav li.teach {
	...
	function getstoryid() {
		var sbmt = document.getElementsByName("lesson_submit");
		console.log(sbmt.disabled);
	...
	function loadcheck() {
		...
		document.getElementById("storyid").value = vals;
		...
		var input_data = document.getElementById("storyid4").getAttribute('value');
		console.log(input_data);
	}
	
	function getlessonid() {
		var checkboxes_story = document.getElementsByName('story[]');
		var vals_story = "";
		for (var i=0, n=checkboxes_story.length;i<n;i++) 
		{
			if (checkboxes_story[i].checked) 
			{
				vals_story += "-"+checkboxes_story[i].value;
			}
		}
		if (vals_story) vals_story = vals_story.substring(1);
	...
	function updatelesson(vals_lesson,vals_story) {
		...
		$.ajax({
			url: "insert_multiple_lesson.php?lesson_ids="+vals_lesson+"&story_id="+vals_story,
			complete: function (response) {
	...
	function check_uncheck(ele) {
		var checkbox_name = ele.getAttribute('value');
	...
	function enableSubmit() {
		...
		var checkboxes_lesson = document.getElementsByName('lesson_ids[]');
		var check_count = 0;
		for (var i=0;i<checkboxes_lesson.length;i++) 
		{
			if (checkboxes_lesson[i].checked) 
			{
				check_count += 1;
...
function getStories($student_id) {
	...
	$sql_story = "select distinct st.sex, s.story_id as id, st.name as name, s.story_type as type, s.recording_context as context, s.story_topic as topic, s.story_category as category, s.relation_science as relation, s.transcript_edited as story from stories s, students st, student_story p where s.story_id = p.story_id and st.student_id = p.student_id and st.student_id = $student_id";
	...
	while($row_story = mysql_fetch_array($result_story))	{
		...
		$story_html_loop = "<div class='w3-row story_div c$type c$context c$topic c$category c$relation' id='s$story_id'>
			<div class='container'>
			  ...
			  <div class='story_row'>
				<p>$story ..<span class='story_details' onclick=\"document.getElementById('f".$story_id."').style.display='block'\" style='width:auto;'><strong>View Full Story</strong></span></p>
			  </div>
			  ...
			  <div class='check'>
				<input class='w3-check' type='checkbox' name='story[]' value='$story_id'> 
			  </div>
			</div>
		</div>";
...
$story_html = getStories($student_id);
echo $story_html;
...
<div id="id02" class="modal">                 
	...
	<form method="post" action="insert_monday_roster.php" id="add_monday">
...
<div id="id03" class="modal">                 
	...
	<form method="post" action="insert_tuesday_roster.php" id="add_monday">
...
<div id="id04" class="modal">                 
	...
	<form method="post" action="insert_wednesday_roster.php" id="add_monday">
...
<div id="id05" class="modal">                 
	...
	<form method="post" action="insert_thursday_roster.php" id="add_monday">
...
<div id="id06" class="modal">                 
	...
	<form method="post" action="insert_friday_roster.php" id="add_monday">
...
<div class="student_profile">
	<img src='student.png' alt='Avatar' class='profile_pic'>
	<div class="profile_details">
<?php
$conn2 = mysql_connect(DB_SERVER,DB_USERNAME,DB_PASSWORD);
if(! $conn2 )
{
	die('Could not connect: ' . mysql_error());
}
$student_id = $_GET['studentid'];
$sql_student = "select distinct name, sex from students where student_id = $student_id";
mysql_select_db('microstory_db');
$result_name = mysql_query( $sql_student, $conn2 );
$name = "";
$sex = "";
